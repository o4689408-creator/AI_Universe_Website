...
// Long enough to read a short explanation, short enough not to feel like
// a stall. Kept in sync with the "quiz-auto-advance" animation duration
// in tailwind.config.ts, which is what actually draws the progress fill
// the reader sees counting down to it.
...
export function QuizSeries({
  ...
  /** Exposed mainly so tests can use a short real delay instead of a
   * fake-timers setup that fights `@testing-library/user-event`'s own
   * internal timing — see components/article/__tests__/QuizSeries.test.tsx. */
  ...
  function handleNext() {
    // Manual click — the reader chooses not to wait out the timer below.
    goToNextOrResults();
  }

  // Automatic advance. This effect only ever schedules a timer while
  // sitting on an already-answered question (`selected !== null`) during
  // the "question" phase — which also means it's naturally a no-op on
  // every render where total === 0, since selected can never leave null
  // without a question having rendered to click an option on in the
  // first place. That's what makes it safe to keep this hook (like every
  // other hook above) unconditionally before the early return below —
  // React requires hooks to run in the same order on every render, and
  // this one now does, without changing when it actually schedules
  // anything.
  //
  // Its cleanup — which React runs before every re-run of this effect,
  // i.e. whenever selected/currentIndex/phase change for *any* reason,
  // including a manual "Next Question" click — clears the pending timer.
  // That's what makes it structurally impossible for both the timer and
  // a manual click to fire the advance twice: by the time either one
  // runs, the other has already been cancelled by this same cleanup, not
  // by a hand-rolled "already advanced" flag that could itself drift out
  // of sync.
  useEffect(() => {
    if (selected === null || phase !== "question") return;
    const timer = setTimeout(goToNextOrResults, autoAdvanceDelayMs);
    return () => clearTimeout(timer);
    // goToNextOrResults intentionally omitted from deps: it's a plain
    // function redefined each render from currentIndex/total, and
    // selected/currentIndex/phase already fully capture every moment
    // this effect needs to reschedule or cancel.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selected, currentIndex, phase]);
                  ...
                  {isAnswered && (
                    <div className="mt-4 flex flex-col items-start gap-2">
                      <button
                        type="button"
                        onClick={handleNext}
                        className="flex items-center gap-1.5 rounded-lg bg-accent px-5 py-2.5 text-body-sm font-semibold text-white transition-all duration-fast ease-out hover:opacity-90"
                      >
                      ...
                      {!prefersReducedMotion && (
                        <div
                          className="h-0.5 w-28 overflow-hidden rounded-full bg-bg-surface-2"
                          aria-hidden="true"
                        >
                          <div key={currentIndex} className="h-full rounded-full bg-accent/50 animate-quiz-auto-advance" />
                        </div>
                      )}
                    </div>
                  )}
