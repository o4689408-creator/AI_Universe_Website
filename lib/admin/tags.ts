...
export async function listTags(query?: string): Promise<TagWithUsage[]> {
  ...
  try {
    const [tags, topics] = await withDbRetry(async () => {
      const collection = await getTagsCollection();
      const filter = query?.trim() ? { name: { $regex: escapeRegExp(query.trim()), $options: "i" } } : {};
      return Promise.all([collection.find(filter).sort({ name: 1 }).toArray(), getAllTopics()]);
    });
...
export async function renameTag(id: string, newName: string): Promise<void> {
  ...
  // Captured once, before any write below, and reused for every step —
  // including a retry of any individual step. If a full-body retry
  // instead re-read the tag document after a partial completion (say
  // the tag's own name/slug got updated but the connection died before
  // the article associations were), it would silently search for the
  // *new* name when looking for articles to update, updating nothing
  // and leaving every affected article's tags array stale.
  ...
  await withDbRetry(async () => {
    ...
    const affectedIds = (await articles.find({ tags: oldName }).project({ _id: 1 }).toArray()).map((d) => d._id);
    if (affectedIds.length === 0) return;
    // $pull and $addToSet are both idempotent — safe even if this
    // exact step is what's being retried.
    await articles.updateMany({ _id: { $in: affectedIds } }, { $pull: { tags: oldName } } as never);
...
export async function deleteTag(id: string): Promise<void> {
  ...
  const tagName = await withDbRetry(async () => {
    const tags = await getTagsCollection();
    const existing = await tags.findOne({ _id: objectId });
    if (!existing) throw new Error("Tag not found.");
    return existing.name;
  ...
  await withDbRetry(async () => {
    const db = await getDb();
    await db.collection("articles").updateMany({ tags: tagName }, { $pull: { tags: tagName } } as never);
  });

  // Deleted last, deliberately: if this specific step is what fails,
  // the recoverable state is "the tag still exists, but no articles
  // reference it anymore" (the admin can just delete it again) rather
  // than "the tag is gone, but articles still carry a phantom name".
  await withDbRetry(async () => {
    const tags = await getTagsCollection();
